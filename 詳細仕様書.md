...
*   **UI部品**:
![ログイン画面](https://raw.githubusercontent.com/xuemeet45/virtualExerciseCoach/refs/heads/main/public/login-s.png)
...
*   **UI部品**:
![新規登録画面](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/newuser-s.png?raw=true)
...
*   **目的**: 運動のリストを表示、ユーザーが運動を選べるようにします。
*   **説明**: 運動リストが表示され、それぞれの運動には名と写真があります。本画面にはマイページ、エクササイズライブラリ、履歴、ログアウトへ行くボタンがあります。
*   **UI部品**:
![メイン画面](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/list-s.png?raw=true)
    ...
    *   ７．`Gtk::Image` (運動の写真、exercises.image_path)
    *   ８．`Gtk::Label` (運動の名、exercises.name)
    *   ９．`Gtk::Button` (詳細を見る)
*   **操作の流れ**:
    1.  画面が表示されたとき (`on_show`)、`DatabaseManager::fetch_exercises`メソッドを呼び出し、データベースから運動のデータを取得します。
    2.  取得した運動データは`Gtk::ListBox`に表示されます。それぞれの行は`Exercise`オブジェクトに対応します。
...
*   **目的**: 運動の詳しい情報を表示させて、ポーズ検出を始める入り口になります。
*   **説明**: 運動の名、説明、やり方、写真などが表示されます。「ポーズ検出開始」ボタンがあります。
*   **UI部品**:
![エクササイズ詳細画面](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/details-s.png?raw=true)
    *   １．`Gtk::Label` (運動の名、exercises.name)
    *   ２．`Gtk::Image` (運動の写真、exercises.image_path)
    *   ３．`Gtk::Label` (固定文字列:”やり方”)
    *   ４．`Gtk::ScrolledWindow` (運動のやり方)
    *   ５．`Gtk::Button` (Try)
*   **操作の流れ**:
    1.  メイン画面から渡された運動IDを使用して、`DatabaseManager::fetch_exercises`を呼び出し、運動情報を取得します。
    2.  取得した情報をそれぞれの`Gtk::Label`や`Gtk::Image`に表示します。
...
*   **目的**: ユーザーのポーズをリアルタイムで表示して、アドバイスをしたり、参照ポーズを記録したりします。
...
*   **操作の流れ**:
    ...
    3.  `update_frame`の中で、カメラから映像のフレームを取得して、TensorFlow Liteモデル (`tflite_interpreter`) を使用してポーズを検出します。
    4.  検出された体のキーポイント (`last_detected_keypoints`) は、データベースから取得した参照ポーズ (`DatabaseManager::fetch_reference_pose`) と比べられます。
...
*   **目的**: ユーザー運動履歴をリストで表示します。
*   **説明**: 運動名、運動した日、期間、状態（例: 実行、試行）などが`Gtk::ListBox`に表示されます。
*   **UI部品**:![エクササイズ履歴画面](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/history-s.png?raw=true)
    ...
    *   ３．`Gtk::Label` (運動名、exercises.name)
    *   ４．`Gtk::Label` (運動した日、exercise_history.session_date)
    *   ５．`Gtk::Label` (運動した期間、exercise_history.duration_minutes)
    *   ６．`Gtk::Label` (状態、exercise_history.status)
    *   ７．`Gtk::Button` (戻る)
*   **操作の流れ**:
    1.  画面が表示されたとき (`on_show`)、`DatabaseManager::fetch_exercise_history`メソッドを呼び出し、今のユーザーの運動履歴を取得します。
    2.  取得した履歴データは`Gtk::ListBox`に表示されます。
    3.  「戻る」ボタンを押すと、`MyPageWindow`か`VirtualFitnessCoachWindow`に戻ります。

#### 2.2.7. マイページ画面 (`MyPageWindow`)
*   **目的**: ユーザー情報や運動の合計データを表示し、関連する設定画面へ行けるようにします。
*   **説明**: ユーザー名、メールアドレス、ユーザー氏名、合計運動時間、合計消費カロリーなどの情報が表示されます。ユーザー情報編集、パスワード変更、履歴表示、ログアウトへ行くボタンがあります。
*   **UI部品**:![マイページ画面](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/mypage-s.png?raw=true)
    ...
    *   ２．`Gtk::Label` (固定文字列:”ユーザー名:”)
    *   ３．`Gtk::Label` (ユーザー名、users.username)
    *   ４．`Gtk::Label` (固定文字列:”メールアドレス:”)
    *   ５．`Gtk::Label` (メールアドレス、users.email)
    *   ６．`Gtk::Label` (固定文字列:”名前:”)
    *   ７．`Gtk::Label` (名前、users.first_name, users.last_name)
    *   ８．`Gtk::Label` (固定文字列:”統計”)
    *   ９．`Gtk::Label` (固定文字列:”総運動時間:”)
    *   １０．`Gtk::Label` (総運動時間、exercise_history.duration_minutesの合計)
    *   １１．`Gtk::Label` (固定文字列:”総消費カロリー:”)
    *   １２．`Gtk::Label` (総消費カロリー、exercise_history.calories_burnedの合計)
    *   １３．`Gtk::Label` (固定文字列:”お気に入りエクササイズ:”)
    *   １４．`Gtk::Label` (お気に入りエクササイズ名、user_profiles.preferencesから取得)
    *   １５．`Gtk::Button` (ユーザー情報編集)
    *   １６．`Gtk::Button` (パスワード変更)
    *   １７．`Gtk::Button` (運動履歴)
    *   １８．`Gtk::Button` (戻る)
    *   １９．`Gtk::Button` (ログアウト)
*   **操作の流れ**:
    1.  画面が表示されたとき (`on_show`)、`DatabaseManager`からユーザー情報と運動の合計データ（例: `fetch_statistics`）を取得して、`Gtk::Label`に表示します。
...
*   **目的**: ユーザーのプロフィール情報を更新します。
*   **説明**: メールアドレス入力フィールド、名入力フィールド、姓入力フィールド、保存ボタン、キャンセルボタンがあります。
*   **UI部品**:![ユーザー情報編集画面](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/userinfochange-s.png?raw=true)
    ...
    *   ２．`Gtk::Label` (ユーザー名、users.username)
    *   ３．`Gtk::Label` (固定文字列:”メールアドレス:”)
    *   ４．`Gtk::Entry` (メールアドレス入力フィールド、デフォルト値はusers.email)
    *   ５．`Gtk::Label` (固定文字列:”名:”)
    *   ６．`Gtk::Entry` (名入力フィールド、デフォルト値はusers.first_name)
    *   ７．`Gtk::Label` (固定文字列:”姓:”)
    *   ８．`Gtk::Entry` (姓入力フィールド、デフォルト値はusers.last_name)
    *   ９．`Gtk::Button` (保存)
    *   １０．`Gtk::Button` (キャンセル)
    *   １１．`Gtk::Label` (エラーメッセージを表示)
*   **操作の流れ**:
    1.  画面が表示されたとき (`on_show`)、ユーザー情報 (`DatabaseManager::fetch_user_profile`) を取得して、それぞれのUI部品に表示します。
...
*   ユーザー情報管理機能 (`ProfileEditWindow`, `DatabaseManager`)
...
#### 3.2.2. ユーザー情報管理機能 (`ProfileEditWindow`, `DatabaseManager`)
*   **ユーザー情報編集**:
    *   **入力**: `User`オブジェクト (更新されユーザー情報が入っています)
    *   **処理**:
        1.  `ProfileEditWindow`でユーザーが入力したユーザー情報を`User`オブジェクトに設定します。
...
*   **運動リスト表示**:
    *   **入力**: なし
    *   **処理**:
        1.  `VirtualFitnessCoachWindow`が始まったときや表示されたときに、`DatabaseManager::fetch_exercises`メソッドを呼び出し、`exercises`テーブルからすべての運動情報を`std::vector<Exercise>`として取得します。
        2.  取得した`Exercise`オブジェクトのリストを`Gtk::ListBox`に表示し、それぞれの運動の`name`と`image_path`を表示します。
    *   **出力**: `std::vector<Exercise>` (運動オブジェクトのリスト)
*   **運動詳細表示**:
    *   **入力**: `exercise_id` (int)
    *   **処理**:
        1.  `ExerciseDetailWindow`は、渡された`exercise_id`を使って`DatabaseManager::fetch_exercises`を呼び出し、特定の運動情報を取得します。
        2.  取得した`Exercise`オブジェクトのそれぞれの情報（`name`, `instructions`、`image_path`など）を画面の`Gtk::Label`や`Gtk::Image`に表示します。
...
*   **処理**:
    ...
    5.  `DatabaseManager::fetch_reference_pose`を呼び出し、今運動している参照ポーズのデータを取得します。
    6.  `last_detected_keypoints`と参照ポーズのそれぞれの関節の距離を計算し、`JOINT_ERROR_THRESHOLD`と比べます。
...
*   **履歴表示**:
    *   **入力**: `user_id` (int)
    *   **処理**:
        1.  `ExerciseHistoryWindow`が表示されたとき (`on_show`)、`DatabaseManager::fetch_exercise_history`メソッドを呼び出し、`user_id`に関係するすべての運動履歴を`std::vector<std::map<std::string, std::string>>`として取得します。
        2.  取得した履歴データは`Gtk::ListBox`に表示されます。
...
*   **処理**:
    1.  `PoseDetectionWindow`で「参照ポーズを記録」ボタンが押されると、`record_reference_pose`メソッドが呼び出されます。
    2.  すでに参照ポーズがあるか`DatabaseManager::fetch_statistics`で確認し、ユーザーに上書きしてもいいか尋ねるダイアログ (`Gtk::MessageDialog`) を表示します。
    ...
    6.  `DatabaseManager::execute_query`で今ある参照ポーズを消し、`DatabaseManager::insert_reference_pose`を使って新しい参照ポーズのそれぞれの関節データ（`x`, `y`, `confidence`）を`reference_poses`テーブルに保存します。
...
*   **データ項目**:
    ...
    *   `created_at`: TIMESTAMP (デフォルト: 現在時刻) - アカウントが作られた日時。
...
*   **データ項目**:
    ...
    *   `created_at`: TIMESTAMP (デフォルト: 現在時刻) - セッションが作られた日時。
...
*   **データ項目**:
    ...
    *   `created_at`: TIMESTAMP (デフォルト: 現在時刻) - プロフィールが作られた日時。
    *   `updated_at`: TIMESTAMP (デフォルト: 現在時刻) - プロフィール情報が最後に更新された日時。
...
*   **データ項目**:
    ...
    *   `created_at`: TIMESTAMP (デフォルト: 現在時刻) - 運動情報が作られた日時。
    *   `updated_at`: TIMESTAMP (デフォルト: 現在時刻) - 運動情報が最後に更新された日時。
...
*   **データ項目**:
    ...
    *   `session_date`: TIMESTAMP (デフォルト: 現在時刻) - 運動が始まった日時。
...
*   **データ項目**:
    *   `id`: SERIAL (主キー) - 参照ポーズデータを区別するための自動で振られる番号。
    *   `exercise_id`: INTEGER (外部キー: `exercises.id`, 必須) - この参照ポーズがどの運動のものかを示すID。
...
![ログインフロー](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/sd/login.png?raw=true)
    ```mermaid
    sequenceDiagram
        actor User
...
![ポーズ検出シーケンス](https://github.com/xuemeet45/virtualExerciseCoach/blob/main/public/sd/pose_detection_sequence.png?raw=true)
    ```mermaid
    sequenceDiagram
        actor User
...
*   **初期値に戻す**:
    *   ユーザー情報編集などで入力した情報が正しくない場合、できる範囲で前の値に戻すか、適切な初期値を設定します。
*   **アプリの終了**:
...
### 7.1. ログインやアクセス権限の管理
*   **パスワードのハッシュ化**:
    *   ユーザーが登録したりパスワードを変えたりするとき、入力されたパスワードはそのままデータベースには保存されません。代わりに、`AuthManager::hash_password`メソッドの中で、SHA-256ハッシュアルゴリズムを使って暗号化されます。
